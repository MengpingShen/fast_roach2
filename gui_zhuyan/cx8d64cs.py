...
bitstream = 'cx8d64cs_2016_Nov_23_1715.bof.gz'
...
#ipalls = struct.unpack('%dB'%len(ipall), ipall)
#for i in range(len(ipalls)):
#    print('%02X ' % ipalls[i]),
#    if i % 8 == 7:
#        print('')
...
def plot_anim():
    ...
    print('CH0:%3d %3d ' % (idx, s0[idx])),
    idx = np.argmax(np.array(s1))
    print('CH1:%3d %3d' % (idx, s1[idx]))
...
if __name__ == '__main__':
    ...
    try:
        ...
        print('Configuring destnation IP address pool ... '),
        fpga.write('ipaddr_pool', ipall)
        print('done')

        print('Configuring channelizer fft_shift, fft_shift=0x%X ... ' % opts.fftshift),
        fpga.write_int('fft_shift', int(opts.fftshift))
        print('done')

        print('Configuring channelizer scale coefficients, gain0=0x%X gain1=0x%X... ' % (opts.gain0, opts.gain1)),
        ...
        fpga.write_int('xfer_tvg', opts.xfertvg)
