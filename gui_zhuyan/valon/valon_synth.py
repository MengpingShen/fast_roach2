...
class Synthesizer:
    ...
    def _pack_freq_registers(self, ncount, frac, mod, dbf, old_bytes):
        dbf_table = {1: 0, 2: 1, 4: 2, 8: 3, 16: 4, 32: 5, 64: 6}
    ...
    def _unpack_freq_registers(self, bytes):
        dbf_rev_table = {0: 1, 1: 2, 2: 4, 3: 8, 4: 16, 5: 32, 6: 64}
    ...
    def get_frequency(self, synth):
        ...
        #print('dbf=%d EPDF=%f ncount=%d frac=%d mod=%d' % (dbf, EPDF, ncount, frac, mod))
    ...
    def set_frequency(self, synth, freq, chan_spacing = 10.):
        ...
        if frac != 0 and mod != 0:
            ...
            frac = 0
            mod = 1
        #print('dbf=%d vco=%f EPDF=%f ncount=%d frac=%d mod=%d' % (dbf, vco, EPDF, ncount, frac, mod))
        self.conn.open()
        bytes = struct.pack('>B', 0x80 | synth)
        self.conn.write(bytes)
